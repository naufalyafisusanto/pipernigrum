<div class="btn-group button-station" id={{ 'action-'.$data->id }} station-id={{ $data->id }}>
    ...
    <div class="dropdown-menu select-action">
        ...
        <a class="dropdown-item @if($data->active && $data->running && $data->rotation == 0){!! 'text-danger button-action' !!}@else{!! 'disabled text-secondary"' !!}@endif" station-action="stop">Stop</a>
        ...
        <div class="dropdown-divider manage"></div>
        <a class="dropdown-item @if($data->active && !$data->running && $data->rotation != 1){!! 'text-magenta button-action' !!}@else{!! 'disabled text-secondary"' !!}@endif" station-action="insert">Insert</a>
        <a class="dropdown-item @if($data->active && !$data->running && $data->rotation != 0){!! 'text-danger button-action' !!}@else{!! 'disabled text-secondary"' !!}@endif" station-action="brake">Brake</a>
        <a class="dropdown-item @if($data->active && !$data->running && $data->rotation != -1){!! 'text-warning button-action' !!}@else{!! 'disabled text-secondary"' !!}@endif" station-action="eject">Eject</a>
        @can('admin')
            <div class="dropdown-divider manage"></div>
            <a class="dropdown-item" onclick="edit_action({{ $data->id }}, 'params')">Edit Params</a>
            <a class="dropdown-item" onclick="edit_action({{ $data->id }}, 'station')">Edit Station</a>
        @endcan
    </div>
    <div class="ml-2">
